...
function init() {
  ...
  $("guess-category").addEventListener("keydown", e => { if (e.key === "Enter") submitGuess(); });
...
function renderGuessInputs() {
  ...
  const parts = state.currentWord ? splitAnswer(state.currentWord.word) : [""];
  ...
  for (let i = 0; i < count; i++) {
    ...
    input.placeholder = count > 1 ? t("guessPartPlaceholder", { n: i + 1 }) : t("guessPlaceholder");
...
function populateCategorySelect() {
  const sel = $("guess-category");
  sel.innerHTML = `<option value="">${escapeHtml(t("categoryPlaceholder"))}</option>`;
...
function renderClue() {
  ...
  let clue = stripVowels(w.word);

  if (state.currentClueRound >= 1 && state.currentClueRound < state.rTot) {
    if (state.currentClueRound === 1) {
      $("attempt-status").textContent = w.help ? t("hintHelp", { help: w.help }) : "";
    } else if (state.currentClueRound === 2) {
      $("attempt-status").textContent = t("hintSubcategory", { category: w.subcategory });
    } else {
      const count = state.currentClueRound - 2;
      clue = revealVowels(w.word, count);
      $("attempt-status").textContent = t("hintVowel");
    }
  } else {
    $("attempt-status").textContent = "";
  }

  $("clue-stage").textContent = clue;
...
function submitGuess() {
  ...
  const answers = splitAnswer(state.currentWord.word);
  const guessCategory = $("guess-category").value;
  ...
  const basePoints = 2 * (state.rTot - state.currentClueRound);
  const partialPoints = Math.max(1, Math.floor(basePoints / 2));

  if (fullCorrect) {
    state.scores[state.currentPlayer] += Math.max(1, basePoints);
  ...
  // En palabras con &, acertar una de las dos palabras da la mitad del puntaje.
  // En cualquier otro caso de acierto parcial (palabra o categoría), también se
  // otorga la mitad: la ronda termina y pasa al siguiente jugador.
  const hasPartial = wordResult.correctCount > 0 || categoryOK;
  if (hasPartial) {
    state.scores[state.currentPlayer] += partialPoints;

    if (answers.length > 1) {
      if (wordResult.correctCount === 1) {
        $("feedback").textContent = t("compoundOneOnly", { word: guesses.find((g, i) => equivalent(g, answers[i])) || "" });
      } else if (wordResult.correctCount === answers.length) {
        $("feedback").textContent = t("compoundWordsButCategory", { points: partialPoints });
      } else {
        $("feedback").textContent = t("partialPoints", { points: partialPoints });
      }
    } else {
      $("feedback").textContent = wordResult.correctCount ? t("wordOnlyPoints", { points: partialPoints }) : t("categoryOnlyPoints", { points: partialPoints });
    }
...
function startTimer() {
  ...
  state.timerId = setInterval(() => {
    ...
    if (state.remainingSeconds <= 0) {
      stopTimer();
      $("feedback").textContent = t("timeout");
      $("feedback").className = "feedback bad";
      advanceFailedAttempt();
    }
  }, 1000);
