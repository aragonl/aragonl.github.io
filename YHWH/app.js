...
function categorySelectedColor(position, count) {
  // Distribuye el tono alrededor del círculo cromático. El desplazamiento
  // evita que los primeros colores queden demasiado próximos entre sí.
  const golden = 137.508;
  const hue = (18 + position * (count > 1 ? 360 / count : golden)) % 360;
...
function renderCategorySelection() {
  ...
  WORD_DATA.forEach((cat, index) => {
    ...
    if (selected.has(index)) {
      const position = state.selectedCategories.indexOf(index);
      card.style.setProperty("--selected-bg", categorySelectedColor(position, Math.max(1, state.selectedCategories.length)));
    }
...
function submitGuess() {
  ...
  // La respuesta se divide en dos bloques de puntuación: PALABRA y CATEGORÍA.
  // Para una respuesta con &, el bloque PALABRA se paga una sola vez, aunque
  // contenga dos objetivos. Así nunca se vuelve a pagar lo que ya adivinó otro.
  let awarded = 0;
  if (newlyCorrectParts.length && !state.wordBlockScored) {
    awarded += componentPoints;
    state.wordBlockScored = true;
  }
...
function advanceFailedAttempt() {
  ...
  if (state.currentClueRoundAttempts >= state.players) {
    ...
    if (state.currentClueRound >= state.rTot) {
      ...
      state.roundWordsCompleted++;
      renderBoard();
      showFeedback(t("revealedWordContinue"), "partial", "revealed");
      return;
    }
  }
