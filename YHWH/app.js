...
function equivalent(a, b) {
  // Las palabras se comparan SIEMPRE de forma independiente de la categoría.
  // Solo se acepta igualdad exacta o una única operación de edición:
  // insertar, eliminar o sustituir una letra.
  ...
  // Inserción/eliminación de una sola letra.
...
function startGame() {
  ...
  renderBoard();
  // La partida comienza con una revelación especial, igual que al completar Rtot.
...
function renderClue() {
  ...
  if (r >= 2) clues.push(`<span class="subcategory-attention">${escapeHtml(t("hintSubcategory", { category: w.subcategory }))}</span>`);
  if (r >= 3) clues.push(`<span class="hint-attention">${escapeHtml(t("hintVowel"))}</span>`);
...
function startComputerReveal(type = "computer", word = null) {
  ...
  $("reveal-title").textContent = type === "exhausted" ? t("revealedUnansweredTitle") : t("specialRound");
}
...
function continueAfterReveal() {
  ...
  if (type === "exhausted") {
    // La palabra se revela, cuenta como completada y recién ahora se continúa.
    state.usedWordIds.add(state.currentWord.id);
    state.roundWordsCompleted++;
    renderBoard();
  ...
  // La primera palabra revelada por el ordenador NO rota el orden: comienza el jugador 1.
  // En las rondas siguientes, el jugador que inició la ronda anterior pasa al final.
  if (type !== "initial") {
