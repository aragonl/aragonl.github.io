...
function equivalent(a, b) {
  // Regla estricta: la respuesta es correcta solamente si coincide o
  // difiere en UNA sola operación de edición (insertar, eliminar o
  // sustituir una letra). La categoría no interviene en esta comparación.
  const aa = normalize(a);
  const bb = normalize(b);
  if (!aa || !bb) return false;
  return editDistance(aa, bb) <= 1;
}
