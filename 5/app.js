...
const DICT_NOMBRES = {
  es: {
    ...
    nombres: new Set([
      "ESAU","ZOROBABEL","ZACARIAS","ZEBEDEO","TIMEO","SAUL",
...
function renderGlossary() {
  ...
  rawData.forEach(item => {
    ...
    // 1. Separar primero por & si existen
    const subWords = item.word.split("&").map(w => w.trim()).filter(Boolean);

    subWords.forEach(subW => {
      // 2. Extraer múltiples palabras si hay conectores (ej: "JESÚS hijo de JOSÉ")
      const parsedList = extractMultipleWords(subW);

      parsedList.forEach(parsed => {
        const sortKey = normalize(parsed.mainText);
        ...
        processedItems.push({
          sortKey: sortKey,
          mainText: parsed.mainText,
          leadingLower: parsed.leadingLower,
          isPN: esNombrePropio(itemCandidate, currentLang),
  ...
  processedItems.forEach(item => {
    const map = item.isPN ? namesMap : wordsMap;
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        leadingLower: item.leadingLower,
        categories: []
      });
    }

    const existing = map.get(key);
    const isDup = existing.categories.some(
      c => c.mainCategory === item.mainCategory && c.subcategory === item.subcategory
    );
  ...
  const buildGlossarySection = (itemsMap, targetContainer, navContainer, prefix) => {
    ...
    const availableLetters = new Set(
      sortedList.map(entry => entry.sortKey.charAt(0).toUpperCase())
    );
      ...
      alphabet.forEach(letter => {
        ...
        if (availableLetters.has(letter)) {
          ...
          btn.addEventListener("click", () => {
            const headerEl = $(`${prefix}-letter-${letter}`);
            if (headerEl) {
    ...
    sortedList.forEach(entry => {
      ...
      if (firstLetter !== currentLetter) {
        ...
        headerContainer.id = `${prefix}-letter-${currentLetter}`;
      ...
      li.style.borderBottom = "1px dotted rgba(128,128,128,0.2)";

      let displayText = entry.leadingLower ? `(${entry.leadingLower}) ${entry.mainText}` : entry.mainText;

      const titleEl = document.createElement("strong");
      titleEl.style.fontSize = "1rem";
      titleEl.textContent = formatWordWithEmojis(displayText);
      li.appendChild(titleEl);

      const catsUl = document.createElement("ul");
      catsUl.style.margin = "2px 0 0 14px";
