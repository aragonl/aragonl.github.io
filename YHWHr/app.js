...
function flattenData() {
  ...
  function traverse(node, mainCategory, difficulty, fortext, currentSubcategory) {
    if (!node) return;

    if (Array.isArray(node.words) && node.words.length > 0) {
      const isNestedCategory = typeof node.words[0] === "object" && node.words[0] !== null && (node.words[0].category || node.words[0].words);

      if (isNestedCategory) {
        node.words.forEach((child) => {
          traverse(child, mainCategory, difficulty, child.fortext || fortext, child.category || currentSubcategory);
        });
      } else {
        node.words.forEach((item, i) => {
          const isObj = typeof item === "object" && item !== null;
          const wordStr = isObj ? item.category : item;
          const wordHelp = isObj ? (item.help ?? "") : (node.help?.[i] ?? "");
          const wordFortext = isObj ? (item.fortext ?? node.fortext ?? fortext) : (node.fortext ?? fortext);

          result.push({
            id: `${mainCategory}-${currentSubcategory}-${wordStr}-${i}`,
  ...
  WORD_DATA.forEach((mainCategory) => {
    traverse(
      mainCategory,
      mainCategory.category,
      mainCategory.dificultad || 1,
      mainCategory.fortext || "",
      mainCategory.category
    );
  });
...
function populateCategorySelect() {
  ...
  const currentMain = state.currentWord.mainCategory;
  const availableSubcategories = [...new Set(
    state.words
      .filter(w => w.mainCategory === currentMain)
      .map(w => w.subcategory)
  )];

  const currentSelection = state.selectedGuessCategory || "";

  availableSubcategories.forEach((subName, index) => {
    const assignedColor = categoryColor(index);

    const isCorrectCategory = state.guessedCategory &&
      normalize(state.currentWord.subcategory) === normalize(subName);
    const isSelected = !state.guessedCategory && normalize(currentSelection) === normalize(subName);
    ...
    btn.addEventListener("click", () => {
      ...
      state.selectedGuessCategory = normalize(currentSelection) === normalize(subName) ? "" : subName;
...
function renderClue() {
  ...
  if (stage >= 1 && w.mainCategory) {
    clues.push(`<span class="subcategory-attention">${escapeHtml(t("hintSubcategory", { category: w.mainCategory }))}</span>`);
  }
  if (stage >= 2) {
...
function renderBoard() {
  ...
  const totalWordsToDiscover = state.words.length;
