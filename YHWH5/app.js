...
const state = {
  ...
  primaryPlayer: 0,   // Secuencia principal (x)
  currentPlayer: 0,   // Secuencia activa (x o y)
...
function equivalent(a, b) {
  ...
  // Palabras con 3 letras o menos tienen que ser exactas
...
function nextWord() {
  ...
  // Para una palabra nueva, el turno vuelve a la secuencia principal (x)
...
function submitGuess() {
  if (!state.currentWord || state.gameOver) return; // Permite responder si está pausado
  ...
  // Acierto Completo: Muestra la pantalla de revelación especial
  if (allDone) {
    stopTimer();
    ...
    // Al adivinar completamente, la secuencia principal avanza al siguiente jugador
    state.primaryPlayer = (state.primaryPlayer + 1) % state.players;
    
    startComputerReveal("guessed", state.currentWord);
    return;
  }
...
function advanceFailedAttempt() {
  // Secuencia secundaria (y): pasa al siguiente jugador
  let nextY = (state.currentPlayer + 1) % state.players;

  // Si dio la vuelta completa y volvió al jugador principal (x), avanzamos la pista
  if (nextY === state.primaryPlayer) {
    state.currentClueRound++;

    if (state.currentClueRound >= state.rTot) {
      stopTimer();
      // Nadie la adivinó: la secuencia principal avanza para la próxima palabra
      state.primaryPlayer = (state.primaryPlayer + 1) % state.players;
...
function startComputerReveal(type = "computer", word = null) {
  ...
  if (type === "exhausted") titleText = t("revealedUnansweredTitle");
  if (type === "guessed") titleText = "¡Palabra Adivinada!";
