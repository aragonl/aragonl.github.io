...
function iniciarNuevaRonda() {
  ...
  // Renderizar subcategorías de la palabra actual
...
function renderizarOpcionesSubcategorias(palabraObjeto) {
  ...
  if (!contenedorOpciones || !palabraObjeto) return;

  contenedorOpciones.innerHTML = '';
  
  // Obtener únicamente las subcategorías asociadas al grupo/categoría de la palabra activa
  const todasLasPalabras = window.palabras[config.idioma] || [];
  const subcategoriasSet = new Set();

  todasLasPalabras.forEach(p => {
    // Agrupa las subcategorías según la categoría superior de la palabra dada
    if (p.categoria === palabraObjeto.categoria && p.subcategoria) {
      subcategoriasSet.add(p.subcategoria);
    }
  });

  // En caso de que la estructura sea por arrays de objetos de categoría:
  if (subcategoriasSet.size === 0 && palabraObjeto.subcategoriasDisponibles) {
    palabraObjeto.subcategoriasDisponibles.forEach(sub => subcategoriasSet.add(sub));
  } else if (subcategoriasSet.size === 0 && palabraObjeto.subcategoria) {
    subcategoriasSet.add(palabraObjeto.subcategoria);
  }

  const subcategoriasArray = Array.from(subcategoriasSet);
  // Mezclar subcategorías obtenidas
